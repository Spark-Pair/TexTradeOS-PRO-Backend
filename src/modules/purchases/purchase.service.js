...
export const savePurchase = (businessId, userId, payload = {}, id = "") => {
  ...
  const row = savePurchaseRecord(businessId, userId, { id: existing?.id || crypto.randomUUID(), purchase_number: existing?.purchase_number || "", purchase_date: purchaseDate, supplier_id: text(payload.supplier_id) || null, supplier_name: supplierName, notes: text(payload.notes), total_amount: normalizedItems.reduce((sum,item) => sum + item.amount, 0), packet_count: normalizedItems.reduce((sum,item) => sum + item.quantity_pkt, 0), created_at: existing?.created_at || timestamp, updated_at: timestamp }, normalizedItems, { allocateNumbers: !existing, year });
  return dto(row);
};
