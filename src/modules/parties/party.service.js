...
const toDto = (kind, row, balance = null) => row ? ({
  ...
  total_debit: Number(balance?.charges || 0),
  total_credit: Number(balance?.credits || 0),
...
export const getPartyStatement = (kind, businessId, id) => {
  const row = getParty(kind, businessId, id);
  if (!row) throw notFound(kind);
  const ledger = getPartyLedger(kind, businessId, id);
  return { party: toDto(kind, row, getPartyBalance(kind, businessId, id)), ...ledger };
};
...
export const createParty = (kind, businessId, payload = {}) => {
  ...
  const row = upsertParty(kind, businessId, {
...
export const toggleParty = (kind, businessId, id) => {
  const existing = getParty(kind, businessId, id);
  if (!existing) throw notFound(kind);
  const row = setPartyStatus(kind, businessId, id, !existing.is_active, new Date().toISOString());
  return toDto(kind, row, getPartyBalance(kind, businessId, row.id));
};
