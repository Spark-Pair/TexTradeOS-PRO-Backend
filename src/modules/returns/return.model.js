...
export const ReturnModel = {
  ...
  soldRemaining(businessId, partyId, articleNo, purchaseNumber, sourceInvoiceId) {
    const args=[businessId,articleNo]; let where="i.business_id=? AND ii.article_no=?";
    if(purchaseNumber){where+=" AND ii.purchase_number=?";args.push(purchaseNumber);} if(partyId){where+=" AND i.customer_id=?";args.push(partyId);} if(sourceInvoiceId){where+=" AND i.id=?";args.push(sourceInvoiceId);}
    ...
    if(purchaseNumber){rwhere+=" AND ri.purchase_number=?";rargs.push(purchaseNumber);} if(partyId){rwhere+=" AND r.party_id=?";rargs.push(partyId);} if(sourceInvoiceId){rwhere+=" AND ri.source_document_id=?";rargs.push(String(sourceInvoiceId));}
  ...
  insertReturn(values) { return db.prepare(`INSERT INTO returns (business_id,created_by,return_number,return_type,return_date,party_id,party_name,linked_invoice_id,linked_purchase_id,stock_action,adjustment_type,adjustment_value,adjustment_input,gross_amount,adjustment_amount,total_amount,total_pcs,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(...values); },
  insertItem(values) { return db.prepare("INSERT INTO return_items (return_id,position,article_no,purchase_number,qr_id,source_document_id,description,pcs,rate,gross_amount,discount,discount_type,discount_amount,amount) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(...values); },
  ...
  find(businessId,id){const row=db.prepare("SELECT * FROM returns WHERE id=? AND business_id=?").get(id,businessId);if(!row)return null;return {...row,articles:db.prepare("SELECT * FROM return_items WHERE return_id=? ORDER BY position").all(id),adjustment:{type:row.adjustment_type,value:row.adjustment_input||row.adjustment_value,total_amount:String(row.total_amount)}};},
  ...
  salesReturnableRows(businessId,partyId){return db.prepare(`SELECT i.id AS source_id,i.invoice_number,ii.article_no,ii.purchase_number,ii.description,ii.rate,ii.discount,ii.discount_type,ii.discount_amount,ii.pcs AS sold_pcs FROM invoice_items ii JOIN invoices i ON i.id=ii.invoice_id WHERE i.business_id=? AND i.customer_id=? ORDER BY i.invoice_date DESC,i.id DESC,ii.id DESC`).all(businessId,String(partyId));},
};
