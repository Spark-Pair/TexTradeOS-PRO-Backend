...
const dataDirectory = path.dirname(path.resolve(env.databasePath));
...
const validateMetadata = (value) => {
  ...
  return {
    schemaVersion: Number(value.schemaVersion || 1), version: value.version, mandatory: value.mandatory,
    publishedAt: value.publishedAt || null, releaseUrl: value.releaseUrl, notes: String(value.notes || ""),
    minimumLauncherVersion: String(value.minimumLauncherVersion || "0.0.0"), frontendImage: value.frontendImage, backendImage: value.backendImage,
  };
};

const readCached = () => { try { return validateMetadata(JSON.parse(fs.readFileSync(statePath, "utf8"))); } catch { return null; } };
export const getPendingMandatoryUpdate = () => {
  ...
  return compareVersions(update.version, env.appVersion) > 0 ? update : null;
};
const writeCached = (metadata) => { fs.mkdirSync(path.dirname(statePath), { recursive: true }); fs.writeFileSync(statePath, `${JSON.stringify(metadata, null, 2)}\n`); };

export const checkForUpdate = async () => {
  const currentVersion = env.appVersion;
  if (env.isDevelopment) return { currentVersion, available: false, online: false, disabled: true, update: null };
  try {
    const response = await fetch(process.env.UPDATE_METADATA_URL || DEFAULT_UPDATE_URL, { headers: { Accept: "application/json", "User-Agent": `TexTradeOS-PRO/${currentVersion}` }, signal: AbortSignal.timeout(10000) });
    ...
    if (available && metadata.mandatory) writeCached(metadata); else if (fs.existsSync(statePath)) fs.rmSync(statePath, { force: true });
    return { currentVersion, available, online: true, update: available ? metadata : null };
  } catch (error) {
    const cached = readCached(); const available = cached ? compareVersions(cached.version, currentVersion) > 0 : false;
    return { currentVersion, available, online: false, checkError: error.message, update: available ? cached : null };
  }
};

export const requestUpdate = async () => {
  if (env.isDevelopment) throw new Error("Updates are disabled in development");
  ...
  fs.writeFileSync(requestPath, `${JSON.stringify({ requestedAt: new Date().toISOString(), ...status.update }, null, 2)}\n`);
