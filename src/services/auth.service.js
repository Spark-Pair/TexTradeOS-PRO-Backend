...
import { newSessionId, signAccessToken, signRefreshToken } from "../auth.js";
...
export const AuthService = {
  login({ username, password, userAgent = "", ip = "" }) {
    ...
    SessionModel.create({ id: sessionId, userId: user.id, refreshToken, userAgent, ip, createdAt: now() });
  ...
  refresh({ refreshToken, sessionId }) {
    ...
    SessionModel.touch(sessionId, now());
    return { accessToken: signAccessToken(user) };
  },

  logout({ sessionId, userId }) {
    if (sessionId) SessionModel.revoke(sessionId, userId, now());
    return { success: true };
  },

  updateShortcuts({ userId, shortcuts }) {
    UserModel.updateShortcuts(userId, shortcuts, now());
    return { shortcuts };
  },
};
