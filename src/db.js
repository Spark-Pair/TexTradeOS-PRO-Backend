...
const backfillBusinessAccessRules = () => {
  ...
  db.transaction(() => {
    businesses.forEach((business) => {
      const referenceData = parseStoredJson(business.reference_data, {});
      const storedRuleData = parseStoredJson(business.rule_data, {});
      ...
      const rolesFromUsers = db.prepare("SELECT DISTINCT role FROM users WHERE business_id = ? AND role IS NOT NULL AND TRIM(role) <> ''").all(business.id).map((row) => row.role);
      ...
      const userRoles = [...new Set([...configuredRoles, ...rolesFromUsers, ...rolesFromRules].map((role) => String(role || "").trim()).filter((role) => role && role !== "developer"))];
      ...
      if (JSON.stringify(nextReferenceData) !== JSON.stringify(referenceData) || JSON.stringify(nextRuleData) !== JSON.stringify(storedRuleData)) {
        update.run(json(nextReferenceData), json(nextRuleData), now(), business.id);
      }
    });
  }).immediate();
};
